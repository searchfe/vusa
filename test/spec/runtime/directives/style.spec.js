...
describe('Directive a-bind:style', () => {
    let vm;
    ...
    it('updates objects deeply', done => {
        ...
        waitForUpdate(() => {
            ...
            expect(vm.$el.style.getPropertyValue('display')).toBe('block');
        }).then(done);
    });
    ...
    it('should not merge for different adjacent elements', done => {
        const vm = new Vue({
            template:
                '<div>' +
                '<section style="color: blue" :style="style" a-if="!bool">12</section>' +
                '<div></div>' +
                '<section style="margin-top: 12px" a-if="bool">34</section>' +
                '</div>',
            data: {
                bool: false,
                style: {
        ...
        let style = vm.$el.children[0].style;
        expect(style.fontSize).toBe('12px');
        expect(style.color).toBe('blue');
        waitForUpdate(() => {
            vm.bool = true;
        }).then(() => {
            // TODO bug san不支持children的style作为引用
            style = vm.$el.children[1].style;
    ...
    it('should not merge for a-if, a-else-if and a-else elements', done => {
        const vm = new Vue({
            template:
                '<div>' +
                '<section style="color: blue" :style="style" a-if="foo"></section>' +
                '<section style="margin-top: 12px" a-else-if="bar"></section>' +
                '<section style="margin-bottom: 24px" a-else></section>' +
                '<div></div>' +
                '</div>',
            data: {
                foo: true,
