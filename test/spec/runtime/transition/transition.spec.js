...
describe('Transition basic', () => {
    ...
    it('basic transition', done => {
        const vm = new Vue({
            template: '<div><transition><div a-if="ok" class="test">foo</div></transition></div>',
            data: {
                ok: true,
            },
        }).$mount(el);
        ...
        expect(replaceComent(vm.$el.innerHTML)).toBe('<div class="test">foo</div>');
        vm.ok = false;
        waitForUpdate(() => {
    ...
    it('named transition', done => {
        const vm = new Vue({
            template: '<div><transition name="test"><div a-if="ok" class="test">foo</div></transition></div>',
            data: {
                ok: true,
            },
        }).$mount(el);
        ...
        const vm = new Vue({
            template: `
                <div>
                    ...
                    <div a-if="ok" class="test">foo</div>
                    </transition>
                </div>
            `,
            data: {
    ...
    it('inline transition object', done => {
        ...
        const vm = new Vue({
            ...
            // 不备注methods不会被解析模板，因此在这里加入methods属性
            methods: {
                enter: enter,
                leave: leave,
            },
    ...
    // ATTENTION san 只有enter 和 leave 其他生命周期存在问题
    ...
    // TODO a-show 不支持transition
    // it('transition events (a-show)', done => {
    //     const onLeaveSpy = jasmine.createSpy('leave');
    //     const onEnterSpy = jasmine.createSpy('enter');
    //     const beforeLeaveSpy = jasmine.createSpy('beforeLeave');
    //     const beforeEnterSpy = jasmine.createSpy('beforeEnter');
    ...
    it('no transition detected', done => {
        ...
        const leaveSpy = jasmine.createSpy('leave');
        const vm = new Vue({
            template: '<div><transition name="nope" @enter="enter" @leave="leave"><div a-if="ok">foo</div></transition></div>',
            data: {
                ok: true,
            },
            methods: {
        ...
        waitForUpdate(() => {
            ...
            expect(replaceComent(vm.$el.innerHTML)).toBe('<div class="nope-leave nope-leave-active">foo</div>');
        }).thenWaitFor(nextFrame).then(() => {
            // expect(vm.$el.innerHTML).toBe('<!---->');
            expect(vm.$el.innerHTML.startsWith('<!--')).toBe(true);
            expect(vm.$el.innerHTML.endsWith('-->')).toBe(true);
            ...
            expect(enterSpy).toHaveBeenCalled();
            expect(replaceComent(vm.$el.innerHTML)).toBe('<div class="nope-enter nope-enter-active">foo</div>');
        }).thenWaitFor(nextFrame).then(() => {
    ...
    // it('transition with a-show', done => {
    //   const vm = new Vue({
    //     template: `
    //       <div>
    //         <transition name="test">
    //           <div a-show="ok" class="test">foo</div>
    //         </transition>
    //       </div>
    //     `,
    //     data: { ok: true }
    ...
    // it('transition with a-show, inside child component', done => {
    //   const vm = new Vue({
    //     template: `
    //       <div>
    //         <test a-show="ok"></test>
    ...
    // it('leaveCancelled (a-show only)', done => {
    ...
    it('animations', done => {
        ...
        // should not apply transition on initial render by default
        expect(replaceComent(vm.$el.innerHTML)).toBe('<div>foo</div>');
        vm.ok = false;
        waitForUpdate(() => {
    ...
    // it('transition on appear with a-show', done => {
    //   const vm = new Vue({
    //     template: `
    //       <div>
    ...
    it('transition on SVG elements', done => {
        const vm = new Vue({
            template: `
                <svg>
                    <transition>
                        <circle cx="0" cy="0" r="10" a-if="ok" class="test"></circle>
                    </transition>
                </svg>
            `,
            data: {
        ...
        // console.log('vm.$el.outerHTML', vm.$el.childNodes[1]);
        // should not apply transition on initial render by default
        // expect(vm.$el.childNodes[0].getAttribute('class')).toBe('test');
        expect(vm.$el.childNodes[1].getAttribute('class')).toBe('test');
        vm.ok = false;
        waitForUpdate(() => {
            // expect(vm.$el.childNodes[0].getAttribute('class')).toBe('test v-leave v-leave-active');
            expect(vm.$el.childNodes[1].getAttribute('class')).toBe('test v-leave v-leave-active');
        }).thenWaitFor(nextFrame).then(() => {
            // expect(vm.$el.childNodes[0].getAttribute('class')).toBe('test v-leave-active v-leave-to');
            expect(vm.$el.childNodes[1].getAttribute('class')).toBe('test v-leave-active v-leave-to');
        }).thenWaitFor(duration + buffer).then(() => {
            ...
            expect(vm.$el.childNodes[1].nodeType).toBe(8); // should be an empty comment node
            // expect(vm.$el.childNodes[1].textContent).toBe('');
            // expect(vm.$el.childNodes[1].textContent).toBe('6');
            vm.ok = true;
        }).then(() => {
            // expect(vm.$el.childNodes[0].getAttribute('class')).toBe('test v-enter v-enter-active');
            expect(vm.$el.childNodes[1].getAttribute('class')).toBe('test v-enter v-enter-active');
        }).thenWaitFor(nextFrame).then(() => {
            // expect(vm.$el.childNodes[0].getAttribute('class')).toBe('test v-enter-active v-enter-to');
            expect(vm.$el.childNodes[1].getAttribute('class')).toBe('test v-enter-active v-enter-to');
        }).thenWaitFor(duration + buffer).then(() => {
            // expect(vm.$el.childNodes[0].getAttribute('class')).toBe('test');
            expect(vm.$el.childNodes[1].getAttribute('class')).toBe('test');
        }).then(done);
    ...
    //     const vm = new Vue({
    //         template: '<div><my-transition><div a-if="ok" class="test">foo</div></my-transition></div>',
    //         data: {
    //             ok: true,
    //         },
    //         components: {
    //             'my-transition': {
    //                 template: '<transition name="test"><slot /></transition>',
    //                 // functional: true,
